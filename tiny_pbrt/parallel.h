//
// Created by 秦浩晨 on 2020/3/16.
//

#ifndef TINY_PBRT_PARALLEL_H
#define TINY_PBRT_PARALLEL_H
    ...
    std::unique_lock<std::mutex> lock(workListMutex);
     ...
     while (!shutdownThreads) {
         if (reportWorkerStats) {
             ReportThreadStats();
             if (--reporterCount == 0)
                 // Once all worker threads have merged their stats, wake up the main thread
            ...
            // Now sleep again
             ...
             for (int64_t index = indexStart; index < indexEnd; ++index) {
                 ...
                 ProfilerState = loop.profileState;
                 if (loop.func1D) {
                     loop.func1D(index);
                 }
...
void ParallelFor(std::function<void(int64_t)> func, int64_t count,
    ...
    // Run iterations immediately if not using threads
...
// Parallel for the 2D data, it's similar to the previous method, but differ in the input data type and details of addressing indices
...
// return the maximum running cores that sys can provide
int NumSystemCores() {
...
// Also, not forget to cleanup the parallel env that you set
